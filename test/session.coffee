...
describe 'Startup', ->
	...
	it 'starts and stops', ->
		...
		sid = await server.createSession 'test', autostart: false
		session = await server.getSession sid
	...
	it 'service survives exit', ->
		...
		server = new Server.SessionServer spec, @tmpdir
		session = await server.getSession await server.activeSessionId()
		assert session
	...
	it 'service survives tree kill', ->
		...
		server = await (new Server.SessionServer spec, @tmpdir)
		session = await server.getSession(await server.activeSessionId())
		assert session
